...
\usepackage{cleveref} % for autoref with capital letter
\bibliography{refs}
...
\abstract{
    This report presents the work developed during the research fellowship at the Institute of Electronics and Informatics Engineering of Aveiro (IEETA), in the context of project UIDB/00127/2020. The research focused on three main areas inside collaborative robotics: human intention anticipation, deep reinforcement learning for robot control, and exploration of robot control techniques beyond MoveIt. In the first area, existing machine learning models were optimised to recognize the objects being grasped by the user by using the right-hand grasping pattern for real-time applications. In the second area, reinforcement learning models were developed to control the robot's movements towards a given goal in real-time. In the third area, alternatives for robot control were studied with a focus on velocity control instead of position control to allow for smoother and more natural movements. All the developed software is available as ROS packages so that it could be easily integrated and used in future works.
}
...
\input{section5_robot_controller}
